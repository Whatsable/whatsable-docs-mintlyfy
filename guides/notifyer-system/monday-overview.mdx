...
**`sent_by` field:** Identifies the sender of an outgoing message — `"WhatsApp App"` when typed in the WhatsApp Business app, the logged-in user's name from the Notifyer chat UI, or the automation/app name for Make, Zapier, n8n, or API. Not available on Trigger 1 (Incoming Chat). You do not need to delete the WhatsApp Business app; [coexistence](/guides/notifyer-system/embedding-process#whatsapp-coexistence) is supported.
