---
title: 'n8n'
description: 'Connect Notifyer System with n8n. Trigger workflows on incoming and outgoing WhatsApp messages — including replies typed in the WhatsApp Business app. Coexistence is supported; you do not delete the app on your phone.'
icon: 'diagram-project'
keywords: ['n8n', 'webhook', 'outgoing messages', 'incoming_message', 'send_by', 'was_message_by_human', 'coexistence', 'WhatsApp Business app']
---
...
<Tip>
  This guide walks you through connecting Notifyer System with n8n to create automated WhatsApp workflows — including triggers that fire when someone types a reply in the WhatsApp Business app.
</Tip>
...
<Steps>
  <Step title="Set up your n8n workflow">
    ...
    3. Add a trigger node of your choice:
      ...
      <Accordion title="WhatsAble trigger node (incoming and outgoing)">
  <Info>
  ...
  <Steps>
    <Step title="Add WhatsAble trigger node">
      ...
      4. From the available trigger options, choose **On new incoming or outgoing message** (the node may also list **On new Incoming message event** — pick the incoming **or outgoing** option if you need phone-app replies)

      <Frame caption="Adding WhatsAble trigger node with incoming or outgoing message selection">
...
<AccordionGroup>
  <Accordion title="Messages not sending?" icon="circle-exclamation">
    <Steps>
      ...
      <Step title="Outgoing events from the WhatsApp Business app">
        Use the **incoming or outgoing** trigger and the **Notifyer System incoming and outgoing messages** credential. Confirm `incoming_message` is `false` and `send_by` is `WhatsApp App` when you type a test reply on your phone. You do not need to delete the WhatsApp Business app.
      </Step>
    </Steps>
  </Accordion>
