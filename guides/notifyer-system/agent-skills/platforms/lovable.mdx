...
description: "Teach Lovable's AI agent how to build Notifyer WhatsApp integrations. Coexistence is supported; webhooks include outgoing messages with send_by and was_message_by_human."
icon: "heart"
keywords: ["Lovable", "webhook", "outgoing", "coexistence", "send_by"]
---
...
- /api:0hqyGRIz/ — webhooks (enable incoming AND outgoing)
...
- Meta WhatsApp Coexistence is supported. Do NOT tell users to delete the WhatsApp Business app.
- Webhooks fire for incoming AND outgoing messages (including hand-typed WhatsApp Business app replies).
  Payload fields: incoming_message (boolean), send_by ("WhatsApp App" | teammate name | automation name),
  was_message_by_human (boolean). last_messages is often a JSON string — parse it.
- Full webhook reference: https://docs.whatsable.app/guides/notifyer-system/api/incoming-message
- Coexistence / embedding: https://docs.whatsable.app/guides/notifyer-system/embedding-process
...
## Coexistence and webhooks
- Do NOT tell users to delete the WhatsApp Business app. Meta WhatsApp Coexistence is supported.
- Webhooks fire for incoming AND outgoing messages, including hand-typed WhatsApp Business app replies.
- Payload: `incoming_message` (boolean), `send_by` (`WhatsApp App` | teammate name | automation), `was_message_by_human` (boolean)
- Docs: https://docs.whatsable.app/guides/notifyer-system/api/incoming-message
...
The webhook fires for incoming AND outgoing messages (including replies typed in the
WhatsApp Business app — coexistence is supported; do not tell users to delete the app).
Insert a row into the messages table with:
  { phone, body, direction: incoming_message ? "inbound" : "outbound",
    send_by, was_message_by_human, received_at }
When incoming_message is false, send_by is "WhatsApp App" (phone), a teammate name
(Notifyer inbox), or an automation name.
```
...
AND outbound messages via webhook (including WhatsApp Business app replies;
incoming_message, send_by, was_message_by_human), and show chat history in the CustomerDetail page.
