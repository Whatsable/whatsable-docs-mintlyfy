---
title: "Webhooks"
description: "Receive real-time WhatsApp events. For Notifyer System (your own WhatsApp Business number), webhooks include outgoing messages typed in the WhatsApp Business app — coexistence is supported; you do not delete the app."
icon: "webhook"
keywords: ["webhook", "outgoing messages", "coexistence", "Notifyer System"]
---

<Warning>
**This page is WhatsAble Bot** (messages sent to WhatsAble's number). The payload table below is incoming-only.

If you use **your own WhatsApp Business number** (Notifyer System): [Meta WhatsApp Coexistence](/guides/notifyer-system/embedding-process#whatsapp-coexistence) is supported — you do **not** delete the WhatsApp Business app. Webhooks fire for **incoming and outgoing** messages, including replies typed on the phone. Fields: `incoming_message`, `send_by` (`WhatsApp App` or teammate name), `was_message_by_human`. Full reference: [Message webhooks](/guides/notifyer-system/api/incoming-message).
</Warning>
...
| **Webhook direction** | Inbound — WhatsAble calls your endpoint | Inbound and **outgoing** — including replies typed in the WhatsApp Business app |

<Info>
If you want to **send** messages to leads or customers from your own WhatsApp Business number, use the [Notifier System](/guides/notifyer-system/embedding-process) instead. Notifyer supports [Meta WhatsApp Coexistence](/guides/notifyer-system/embedding-process#whatsapp-coexistence) — you do **not** delete the WhatsApp Business app. Outgoing events (phone app, team inbox, automations) are documented in [Message webhooks](/guides/notifyer-system/api/incoming-message), with `incoming_message`, `send_by`, and `was_message_by_human`.
</Info>
